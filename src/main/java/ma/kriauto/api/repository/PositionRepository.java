...
public interface PositionRepository extends JpaRepository<Position, Long> {
	...
	@Query("SELECT p FROM Position p where p.fixtime = (SELECT MAX(p.fixtime) FROM Position p WHERE p.deviceid=:deviceid and to_char(p.fixtime,'YYYY-MM-DD') <=:date AND p.valid = true)")
	public List<Position> fetchLastPositionByDeviceIdAndDate(@Param("deviceid") Integer deviceid, @Param("date") String date);
	
	@Query("SELECT p FROM Position p where p.fixtime = (SELECT MAX(p.fixtime) FROM Position p WHERE p.deviceid=:deviceid AND p.valid = true)")
