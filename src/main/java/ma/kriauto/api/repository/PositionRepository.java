...
public interface PositionRepository extends JpaRepository<Position, Long> {
	...
	@Query("SELECT p FROM Position p where p.fixtime = (SELECT MAX(p.fixtime) FROM Position p WHERE p.deviceid=:deviceid AND to_char(p.fixtime,'YYYY-MM-DD') <=:date) AND p.deviceid=:deviceid AND to_char(p.fixtime,'YYYY-MM-DD') <=:date ORDER BY p.fixtime DESC")
	...
	@Query("SELECT p  FROM Position p where p.speed = (SELECT MAX(p.speed) FROM Position p WHERE to_char(p.fixtime,'YYYY-MM-DD') =:date AND p.deviceid=:deviceid) AND to_char(p.fixtime,'YYYY-MM-DD') =:date AND p.deviceid=:deviceid ORDER BY p.fixtime DESC")
	List<Position> fetchMaxSpeedDeviceIdAndDate(@Param("date") String date, @Param("deviceid") Integer deviceid);
	
	@Query("SELECT p  FROM Position p where p.speed = (SELECT MAX(p.speed) FROM Position p WHERE to_char(p.fixtime,'YYYY-MM-DD HH:mm:ss') >=:date AND p.deviceid=:deviceid) AND p.deviceid=:deviceid AND to_char(p.fixtime,'YYYY-MM-DD HH:mm:ss') >=:date ORDER BY p.fixtime DESC")
